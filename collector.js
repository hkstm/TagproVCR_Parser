...
//  report({status: 'initialized'});
 ...
 function tagprosocketongroupId(data){
  group = data || '';
 };
 
 function tagprosocketonmap(data){
  // console.log('data in map: ' + JSON.stringify);
  if(mapTiles == '')
  {
   if(data.info)
    mapInfo = data.info;
 ...
 function tagprosocketonend(data){
  finalize(true, true);
 };
 function stop()
 {
 ...
 function tagprosocketondisconnect(){
     stop();
 };

 function tagprosocketonobject(data){
 ...
 function tagprosocketonplayerLeft(id) {
  var now = Date.now();
  var player = players[id];
  if(player)
  {
 ...
 function tagprosocketonp(data){
  ...
  } else time = ((now - lastNow) * .06 >>> 0) + lastTime;
